...
pub fn print_benchmark_help() {
    println!(
        "usage: cargo run --release --bin classify_bench -- [--text|--json] [--families iid,structured] [--predictor-max-generations <n>] [--oracle-max-generations <n>] [--randomized] [--seed <n>] [--cases-per-family <n>] [--exhaustive-5x5] [--oracle-max-jump] [--oracle-representative-case] [--oracle-runtime-target-generation <n>] [--progress] [--diagnostic]"
    );
    ...
    println!("  --diagnostic        report mismatches without a failing exit status");
}
...
mod tests {
    ...
    fn parses_max_generations_for_app() {
        let config = parse_from(vec!["--max-generations".to_string(), "2048".to_string()])
            .or_invariant("required value");
    ...
    fn rejects_unknown_argument() {
        let error = parse_from(vec!["--bogus".to_string()]).error_or_invariant("expected error");
        assert_eq!(
            error,
    ...
    fn rejects_zero_dimensions() {
        let error = parse_from(vec!["--height".to_string(), "0".to_string()])
    ...
    fn rejects_zero_step_generations() {
        let error = parse_from(vec!["--step-generations".to_string(), "0".to_string()])
            .error_or_invariant("expected error");
        assert_eq!(
            error,
    ...
    fn rejects_legacy_fast_forward_argument() {
        let error = parse_from(vec!["--fast-forward".to_string(), "64".to_string()])
            .error_or_invariant("expected error");
        assert_eq!(
            error,
    ...
    fn rejects_unknown_pattern() {
        let error = parse_from(vec!["--pattern".to_string(), "not_a_pattern".to_string()])
            .error_or_invariant("expected error");
        assert_eq!(
            error,
    ...
    fn parses_oracle_max_jump_flag() {
        let config = parse_benchmark_from(vec!["--oracle-max-jump".to_string()])
            .or_invariant("required value");
        assert_eq!(
            config,
            BenchmarkCliConfig {
    ...
    fn parses_progress_flag() {
        let config =
            parse_benchmark_from(vec!["--progress".to_string()]).or_invariant("required value");
        assert_eq!(
            config,
            BenchmarkCliConfig {
                format: BenchmarkFormat::Text,
    ...
    #[test]
    fn parses_diagnostic_benchmark_flag() {
        let config =
            parse_benchmark_from(vec!["--diagnostic".to_string()]).or_invariant("required value");
    ...
    fn benchmark_help_is_reported() {
        let error =
            parse_benchmark_from(vec!["--help".to_string()]).error_or_invariant("expected error");
    ...
    fn benchmark_rejects_unknown_argument() {
        let error =
            parse_benchmark_from(vec!["--bogus".to_string()]).error_or_invariant("expected error");
        assert_eq!(
            error,
            CliAction::Error("unknown argument: --bogus".to_string())
        );
    ...
    fn benchmark_rejects_missing_families_value() {
        let error = parse_benchmark_from(vec!["--families".to_string()])
            .error_or_invariant("expected error");
        assert_eq!(
            error,
    ...
    fn parses_oracle_representative_case_flag() {
        let config = parse_benchmark_from(vec!["--oracle-representative-case".to_string()])
            .or_invariant("required value");
        assert_eq!(
            config,
            BenchmarkCliConfig {
