...
impl HashLifeEngine {
    ...
    pub(super) fn compact_discovered_jump_tasks<const N: usize>(
        ...
        for child_key in child_tasks {
            if let Some(index) = compacted[..unique_count]
                .iter()
                .position(|existing| existing.task.key == child_key.key)
            {
    ...
    pub(super) fn build_recursive_parent_chunk_child_states<const C: usize>(
        ...
        for parent_record in &mut parent_records[..parent_count] {
            let parent_child_start = child_arena.len();
            let next_exp = parent_record.next_exp;
            ...
            for overlap_index in REVERSED_OVERLAP_INDEX {
                let child = parent_record.overlaps[overlap_index];
                let query = JumpQuery {
                    node: child,
                    step_exp: next_exp,
                };

                let query_index = u16::try_from(
                    if let Some(existing) = chunk_queries[..chunk_unique_count]
                        .iter()
                ...
                .or_invariant("recursive query chunk exceeded u16 capacity");
                    ...
                    .position(|&existing| existing == query_index)
                {
                    ...
                    parent_arena_offsets[parent_unique_count] = u8::try_from(parent_unique_count)
                        .or_invariant("recursive parent exceeded u8 child capacity");
                    parent_unique_count += 1;
                    if !self.try_push_transient(
                        child_arena,
                        RecursiveParentChildRef {
                            query_index,
                            duplicate_count: 1,
                        },
                    ) {
                        return (chunk_child_states, 0);
                    }
                }
            }

            parent_record.child_arena_start = u16::try_from(parent_child_start)
                .or_invariant("recursive child arena exceeded u16 capacity");
            parent_record.child_arena_len = u8::try_from(child_arena.len() - parent_child_start)
                .or_invariant("recursive parent exceeded u8 child capacity");
        }
    ...
    pub(in crate::hashlife) fn discovered_jump_tasks_from_nodes<const N: usize>(
        ...
        for lane in 0..N {
            ...
            if let Some(previous_lane) = child_nodes[..lane]
                .iter()
                .position(|previous| *previous == child)
            {
                child_keys[lane] = child_keys[previous_lane];
                continue;
            }
    ...
    pub(super) fn probe_jump_cache_presence_batch<const N: usize>(
        ...
        self.stats.canonical_fallback.jump_presence_probe_batches += 1;
        self.stats.canonical_fallback.jump_presence_probe_lanes += active_lanes;
        self.stats.scheduler.scheduler_probe_batches += 1;
        for lane in 0..active_lanes {
            fingerprints[lane] = keys[lane].fingerprint();
        }
        let (cached, probe_accounting) =
            Self::probe_table_many(&self.result_caches.jump, keys, &fingerprints, active_lanes);
        ...
        for lane in 0..active_lanes {
            present[lane] = cached[lane].is_some()
                || self
                    .active_jump_results
                    .get_with_fingerprint(&keys[lane], fingerprints[lane])
                    .is_some();
            hit_count += usize::from(present[lane]);
        }
        self.stats.canonical_fallback.jump_presence_probe_hits += hit_count;
        present
    }
}
