...
#[cfg(test)]
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub(crate) struct HashLifeSessionAdvanceProfile {
    pub advance_root_calls: usize,
    pub advance_root_generations: u64,
    pub advance_root_extract_reembed_calls: usize,
    pub advance_root_pow2_segments: usize,
    pub center_expand_calls: usize,
    pub advance_root_elapsed_us: u128,
    pub advance_root_gc_elapsed_us: u128,
}
...
pub struct HashLifeSession {
    ...
    #[cfg(test)]
    advance_profile: HashLifeSessionAdvanceProfile,
}
...
impl HashLifeSession {
    ...
    pub fn advance_root(&mut self, generations: u64) -> SessionAdvanceStats {
        ...
        #[cfg(test)]
        let profile_before = self.advance_profile;
        ...
        while remaining != 0 {
            ...
            #[cfg(test)]
            {
                self.advance_profile.advance_root_pow2_segments += 1;
            }
            remaining -= step;
        }
        #[cfg(test)]
        ...
        #[cfg(test)]
        {
            self.advance_profile.advance_root_gc_elapsed_us += gc_started.elapsed().as_micros();
            self.advance_profile.advance_root_calls += 1;
            self.advance_profile.advance_root_generations = self
                .advance_profile
                .advance_root_generations
                .saturating_add(generations);
            self.advance_profile.advance_root_elapsed_us += started.elapsed().as_micros();
            let _ = profile_before;
        }
    ...
    fn ensure_centered_capacity(&mut self, desired_step_exp: u32) {
        ...
        loop {
            ...
            #[cfg(test)]
            {
                self.advance_profile.center_expand_calls += 1;
            }
        }
    }
    ...
    #[cfg(test)]
    pub(crate) fn advance_profile(&self) -> HashLifeSessionAdvanceProfile {
