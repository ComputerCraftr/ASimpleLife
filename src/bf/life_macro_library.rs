...
/// Placement metadata used only by the non-executable test scaffold.
///
/// These templates are not verified physical Life logic components and are not
/// consumed by production BF-to-Life compilation.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub fn life_macro_template(kind: LifeMacroKind) -> &'static LifeMacroTemplate {
    LIFE_MACRO_TEMPLATES
        ...
        .or_invariant("missing macro template")
}
...
pub fn instantiate_macro_cells(instance: &LifeMacroInstance) -> Vec<Cell> {
    ...
    template
        ...
        .iter()
        .map(|&(x, y)| {
            let (tx, ty) = transform_cell(
                (x - template.anchor.0, y - template.anchor.1),
