...
pub(super) fn mask_literal(bits: u32) -> String {
    if bits == 0 {
        ...
        "((UINT64_C(1) << 63) - UINT64_C(1))".to_string()
    } else {
        format!("UINT64_C({})", (1u64 << bits) - 1)
    }
}

pub(super) fn wrap_ptr_expr(offset: crate::bf::BfOffset) -> String {
    let offset = normalized_c_offset(offset);
    format!("bf_wrap_ptr(ptr, {offset}, BF_TAPE_LEN)")
}

pub(super) fn normalized_c_offset(offset: crate::bf::BfOffset) -> crate::bf::BfOffset {
    super::tape::centered_offset(offset, super::BF_C_TAPE_LEN)
}
...
pub(super) fn split_output_stmt(opts: CodegenOpts) -> &'static str {
    match (opts.io_mode, opts.cell_sign) {
        (IoMode::Char, _) => "putchar((unsigned char)(((uint64_t)tape[ptr]) & BF_OUTPUT_MASK));",
        (IoMode::Number, CellSign::Signed) => {
