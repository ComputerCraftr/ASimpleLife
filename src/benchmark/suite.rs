...
fn seeded_iid_cases(run_seed: u64, cases_per_family: usize) -> Vec<BenchmarkCase> {
    ...
    for idx in 0..cases_per_family {
        let case_seed = derive_seed(run_seed, [0x1000_0000_0000_0000, case_index(idx)]);
        let size = pick_from(&[16_i64, 32, 64, 128, 256], case_seed, 0);
        let density = pick_from(&[5_u32, 10, 20, 30, 50], case_seed, 1);
        let grid_seed = derive_seed(case_seed, [0xA11D_D00D]);
        ...
        cases.push(seeded_case(
            ...
            i32::try_from(size).or_invariant("benchmark size exceeded i32"),
            density,
            grid,
            Some(format!(
...
fn seeded_structured_cases(run_seed: u64, cases_per_family: usize) -> Vec<BenchmarkCase> {
    let mut cases = Vec::new();
    for idx in 0..cases_per_family {
        let case_seed = derive_seed(run_seed, [0x2000_0000_0000_0000, case_index(idx)]);
        let size = pick_from(&[16_i64, 32, 64, 128, 192], case_seed, 0);
        let grid_seed = derive_seed(case_seed, [0x51DE_BAAD]);
...
fn seeded_clustered_cases(run_seed: u64, cases_per_family: usize) -> Vec<BenchmarkCase> {
    let mut cases = Vec::new();
    for idx in 0..cases_per_family {
        let case_seed = derive_seed(run_seed, [0x3000_0000_0000_0000, case_index(idx)]);
        let size = pick_from(&[24_i64, 48, 96, 144], case_seed, 0);
        let density = pick_from(&[8_u32, 12, 18, 24, 30], case_seed, 1);
        let grid_seed = derive_seed(case_seed, [0xC1A5_7EED]);
...
fn seeded_delayed_cases(run_seed: u64, cases_per_family: usize) -> Vec<BenchmarkCase> {
    let mut cases = Vec::new();
    for idx in 0..cases_per_family {
        let case_seed = derive_seed(run_seed, [0x4000_0000_0000_0000, case_index(idx)]);
        ...
        cases.push(seeded_case(
            ...
            .or_invariant("benchmark size exceeded i32"),
            estimate_density_percent(&grid),
            grid,
            Some(format!(
...
fn seeded_gadget_cases(run_seed: u64, cases_per_family: usize) -> Vec<BenchmarkCase> {
    ...
    for idx in 0..cases_per_family {
        let case_seed = derive_seed(run_seed, [0x5000_0000_0000_0000, case_index(idx)]);
        let dx = bounded_seed_coord(case_seed % 64);
        let dy = bounded_seed_coord((case_seed >> 6) % 48);
        let block_dx = 20 + bounded_seed_coord((case_seed >> 12) % 32);
        let block_dy = 8 + bounded_seed_coord((case_seed >> 17) % 32);
        let blink_dx = 40 + bounded_seed_coord((case_seed >> 22) % 32);
        let blink_dy = 16 + bounded_seed_coord((case_seed >> 27) % 32);
        ...
        cases.push(seeded_case(format!("gadget_seed{case_seed}_g{dx}_{dy}_b{block_dx}_{block_dy}_l{blink_dx}_{blink_dy}"), BenchmarkFamily::ComputationalGadget, i32::try_from(grid.bounds().map(|(min_x, _, max_x, _)| max_x - min_x + 1).unwrap_or(0)).or_invariant("benchmark size exceeded i32"), estimate_density_percent(&grid), grid, Some(format!("family=gadget,glider=({dx},{dy}),block=({block_dx},{block_dy}),blinker=({blink_dx},{blink_dy}),seed={case_seed}"))));
    }
    cases
}
fn seeded_emitter_cases(run_seed: u64, cases_per_family: usize) -> Vec<BenchmarkCase> {
    let mut cases = Vec::new();
    let gun = pattern_by_name("gosper_glider_gun").or_invariant("required value");
    let blocker = pattern_by_name("block").or_invariant("required value");
    for idx in 0..cases_per_family {
        let case_seed = derive_seed(run_seed, [0x6000_0000_0000_0000, case_index(idx)]);
        let block_dx = 90 + bounded_seed_coord((case_seed >> 8) % 160);
        let block_dy = 10 + bounded_seed_coord((case_seed >> 20) % 80);
...
fn time_seed() -> u64 {
    match SystemTime::now().duration_since(UNIX_EPOCH) {
        Ok(duration) => derive_seed(duration.as_secs(), [u64::from(duration.subsec_nanos())]),
        Err(_) => mix64(0xD1CE_F00D_1234_5678),
    }
}

pub(super) fn exhaustive_small_box_cases() -> Vec<BenchmarkCase> {
    let mut cases = Vec::new();
...
fn seeded_translated_pattern_cases(
    ...
    let mut cases = Vec::new();
    for idx in 0..cases_per_family {
        let case_seed = derive_seed(run_seed, [salt, case_index(idx)]);
        let name = pick_from(names, case_seed, 0);
        let base = pattern_by_name(name).or_invariant("required value");
        let (dx, dy) = seeded_translation(case_seed);
        let grid = base.translated(dx, dy);
        cases.push(seeded_case(
...
fn seeded_translated_grid_cases(
    ...
    for idx in 0..cases_per_family {
        let case_seed = derive_seed(run_seed, [salt, case_index(idx)]);
        let (name, base) = &bases[seeded_index(derive_seed(case_seed, [0]), bases.len())];
        let (dx, dy) = seeded_translation(case_seed);
        let grid = base.translated(dx, dy);
        cases.push(seeded_case(
            format!("{family}_{name}_dx{dx}_dy{dy}_seed{case_seed}"),
...
pub(crate) fn distant_glider_trigger(
    distance: Coord,
    target: BitGrid,
    target_origin: Cell,
) -> BitGrid {
    let glider = [(1, 0), (2, 1), (0, 2), (1, 2), (2, 2)];
    let mut grid = target.translated(target_origin.0, target_origin.1);
    for (x, y) in glider {
        grid.set(
            x + target_origin.0 - distance,
            y + target_origin.1 - distance,
            true,
        );
    }
    grid
...
fn seeded_translation(seed: u64) -> (Coord, Coord) {
    (
        bounded_seed_coord((seed >> 8) % 48),
        bounded_seed_coord((seed >> 20) % 48),
    )
}
pub(super) fn estimate_density_percent(grid: &BitGrid) -> u32 {
    let Some((min_x, min_y, max_x, max_y)) = grid.bounds() else {
        return 0;
    };
    let width = u128::try_from((i128::from(max_x) - i128::from(min_x) + 1).max(1))
        .or_invariant("positive benchmark width exceeded u128");
    let height = u128::try_from((i128::from(max_y) - i128::from(min_y) + 1).max(1))
        .or_invariant("positive benchmark height exceeded u128");
    let area = width.saturating_mul(height).max(1);
    let density = (grid.population() as u128)
        .saturating_mul(100)
        .saturating_div(area);
    u32::try_from(density.min(100)).or_invariant("density percentage is capped at 100")
}
...
fn pick_from<T: Copy>(values: &[T], seed: u64, salt: u64) -> T {
    values[seeded_index(derive_seed(seed, [salt]), values.len())]
}

fn bounded_seed_coord(value: u64) -> Coord {
    Coord::try_from(value).or_invariant("bounded seed coordinate exceeded Coord")
}

fn case_index(index: usize) -> u64 {
    u64::try_from(index).or_invariant("benchmark case index exceeded u64")
}

fn seeded_index(seed: u64, len: usize) -> usize {
    let len_u64 = u64::try_from(len).or_invariant("collection length exceeded u64");
...
#[cfg(test)]
mod wide_density_tests {
    use super::*;

    #[test]
    fn density_estimate_handles_full_coordinate_width_without_overflow() {
        let grid = BitGrid::from_cells(&[(Coord::MIN, 0), (Coord::MAX, 0)]);
        assert_eq!(estimate_density_percent(&grid), 0);
    }
}
