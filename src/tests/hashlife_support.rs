...
use crate::hashlife::{HashLifeDiagnosticSummary, HashLifeRuntimeStats};
...
pub(super) fn assert_normalized_grids_eq(label: &str, actual: &BitGrid, expected: &BitGrid) {
...
pub(super) fn assert_grids_eq(label: &str, actual: &BitGrid, expected: &BitGrid) {
    assert_eq!(
        actual,
        expected,
...
pub(super) fn assert_hashlife_probe_ok(label: &str, ok: bool, stats: HashLifeRuntimeStats) {
    assert!(ok, "{label}: verification probe failed\nstats={stats:?}");
}

pub(super) fn assert_hashlife_runtime_stat(
    label: &str,
    stats: HashLifeRuntimeStats,
    predicate: bool,
) {
    assert!(
        predicate,
        "{label}: runtime invariant failed\nstats={stats:?}"
    );
}
...
pub(super) fn assert_hashlife_matches_stepper(grid: BitGrid, generations: u64) {
    ...
    assert_grids_eq(
        &format!("HashLife vs stepper mismatch at generations={generations}"),
        &advanced,
        game.grid(),
    );
}
...
pub(super) fn grid_from_mask(width: Coord, height: Coord, mask: u32) -> BitGrid {
    ...
    for y in 0..height {
        for x in 0..width {
            let bit = u32::try_from(y * width + x).or_invariant("grid mask index exceeded u32");
            if (mask >> bit) & 1 == 1 {
                cells.push((x, y));
            }
        }
...
pub(super) fn mirror_grid_x(grid: &BitGrid) -> BitGrid {
    let (min_x, _, max_x, _) = grid.bounds().or_invariant("grid should be non-empty");
    let mirrored = grid
        .live_cells()
        .into_iter()
...
pub(super) fn build_hashlife_structured_symmetry_grid() -> BitGrid {
    let mut cells = pattern_by_name("gosper_glider_gun")
        .or_invariant("required value")
        .live_cells();
    cells.extend(
        mirror_grid_x(&pattern_by_name("gosper_glider_gun").or_invariant("required value"))
...
pub(super) fn print_hashlife_summary(
    ...
    eprintln!(
        "{} total_us={} nodes={} retained_roots={} nodes_match_intern={} dependency_stalls={} jump_result_hit_rate={:.3} jump_result_misses={} oriented_result_hit_rate={:.3} root_result_hit_rate={:.3} jump_presence_hit_rate={:.3} overlap_hit_rate={:.3} overlap_local_reuse_rate={:.3} symmetry_gate_allow_rate={:.3} symmetry_gate_canonical_cache_bypasses={} structural_fast_path_hit_rate={:.3} canonical_node_cache_hit_rate={:.3} canonical_packed_cache_hit_rate={:.3} canonical_oriented_cache_hit_rate={:.3} direct_parent_winner_hit_rate={:.3} direct_parent_cached_result_hits={} direct_parent_winner_fallbacks={} blocked_symmetry_scan_fallbacks={} admitted_symmetry_scan_fallbacks={} total_symmetry_scan_fallbacks={} symmetry_jump_result_hits={} simd_lane_coverage={:.3} scalar_commit_ratio={:.3} probes_per_scheduler_task={:.3} recursive_overlap_batch_rate={:.3} gc_reclaim_ratio={:.3} gc_compact_ratio={:.3} gc_transient_pressure_entries_before={} gc_canonical_cache_entries_before={} gc_skipped_with_transient_growth={} canonical_packed_cache_entries={} canonical_oriented_cache_entries={} direct_parent_cache_entries={} structural_fast_path_cache_entries={} packed_structural_fast_path_cache_entries={} oriented_result_cache_entries={} packed_transform_intern_entries={} packed_d4_misses={} packed_inverse_transform_hits={} packed_recursive_transform_hits={} packed_recursive_transform_misses={} packed_overlap_outputs={} packed_result_materializations={} session_full_grid_materializations={} embedded_result_full_extractions={} clipped_viewport_extractions={} checkpoint_cell_materializations={} oracle_confirmation_materializations={} transformed_node_materializations={} gc_reason={} gc_runs={} gc_skips={}",
        label,
        elapsed.as_micros(),
        summary.overview.total_nodes,
        summary.overview.retained_roots,
        summary.overview.nodes_match_intern,
        summary.overview.dependency_stalls,
        summary.cache_rates.jump_result_hit_rate,
        summary.cache_rates.jump_result_miss_count,
        summary.cache_rates.oriented_result_hit_rate,
        summary.cache_rates.root_result_hit_rate,
        summary.cache_rates.jump_presence_hit_rate,
        summary.cache_rates.overlap_hit_rate,
        summary.cache_rates.overlap_local_reuse_rate,
        summary.cache_rates.symmetry_gate_allow_rate,
        summary.cache_rates.symmetry_gate_canonical_cache_bypasses,
        summary.cache_rates.structural_fast_path_hit_rate,
        summary.cache_rates.canonical_node_cache_hit_rate,
        summary.cache_rates.canonical_packed_cache_hit_rate,
        summary.cache_rates.canonical_oriented_cache_hit_rate,
        summary.cache_rates.direct_parent_winner_hit_rate,
        summary.cache_rates.direct_parent_cached_result_hits,
        summary.cache_rates.direct_parent_winner_fallbacks,
        summary.fallbacks.blocked_symmetry_scan_fallbacks,
        summary.fallbacks.admitted_symmetry_scan_fallbacks,
        summary.fallbacks.total_symmetry_scan_fallbacks,
        summary.fallbacks.symmetry_jump_result_hits,
        summary.work_rates.simd_lane_coverage,
        summary.work_rates.scalar_commit_ratio,
        summary.work_rates.probes_per_scheduler_task,
        summary.work_rates.recursive_overlap_batch_rate,
        summary.gc.gc_reclaim_ratio,
        summary.gc.gc_compact_ratio,
        summary.gc.gc_transient_pressure_entries_before,
        summary.gc.gc_canonical_cache_entries_before,
        summary.gc.gc_skipped_with_transient_growth,
        summary.cache_sizes.canonical_packed_cache_entries,
        summary.cache_sizes.canonical_oriented_cache_entries,
        summary.cache_sizes.direct_parent_cache_entries,
        summary.cache_sizes.structural_fast_path_cache_entries,
        summary
            .cache_sizes
            .packed_structural_fast_path_cache_entries,
        summary.cache_sizes.oriented_result_cache_entries,
        summary.cache_sizes.packed_transform_intern_entries,
        summary.transforms.packed_d4_canonicalization_misses,
        summary.transforms.packed_inverse_transform_hits,
        summary.transforms.packed_recursive_transform_hits,
        summary.transforms.packed_recursive_transform_misses,
        summary.transforms.packed_overlap_outputs_produced,
        summary.transforms.packed_cache_result_materializations,
        summary.materializations.session_full_grid_materializations,
        summary.materializations.embedded_result_full_extractions,
        summary.materializations.clipped_viewport_extractions,
        summary.materializations.checkpoint_cell_materializations,
        summary
            .materializations
            .oracle_confirmation_materializations,
        summary.materializations.transformed_node_materializations,
        summary.gc.gc_reason,
        summary.gc.gc_runs,
        summary.gc.gc_skips
    );
}
...
pub(super) fn assert_stepper_matches_single_step(grid: &BitGrid) {
    ...
    assert_normalized_grids_eq(
        "single-step HashLife result should match scalar stepper",
        &advanced,
        &step_grid(grid),
    );
}
