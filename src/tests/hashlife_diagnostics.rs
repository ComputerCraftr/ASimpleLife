...
use super::hashlife_support::{
    GEN_DEEP_DIAGNOSTIC, GEN_LARGE_PRIME, GEN_MIRROR_REUSE, GEN_STRUCTURED_PROMOTION_DIAGNOSTIC,
    GEN_STRUCTURED_TUNING_DIAGNOSTIC, LARGE_SOUP_DIM, SEED_CANONICAL_RESULT_INSERT,
    SMALL_SOUP_FILL, SYMMETRY_GATE_DEFAULT_LEVEL, SYMMETRY_GATE_DEFAULT_POPULATION,
    SYMMETRY_GATE_UNGATED_LEVEL, SYMMETRY_GATE_UNGATED_POPULATION, SYMMETRY_GATE_WIDE_LEVEL,
    SYMMETRY_GATE_WIDE_POPULATION, XL_SOUP_DIM, build_hashlife_step0_stress_grid,
    build_hashlife_structured_symmetry_grid, mirror_grid_x, print_hashlife_gate_comparison,
    print_hashlife_summary,
};

const SYMMETRY_GATE_CONFIGS: [(&str, u32, u64); 3] = [
    (
        "default",
        SYMMETRY_GATE_DEFAULT_LEVEL,
        SYMMETRY_GATE_DEFAULT_POPULATION,
    ),
    (
        "wider",
    ...
    (
        "ungated",
        SYMMETRY_GATE_UNGATED_LEVEL,
        SYMMETRY_GATE_UNGATED_POPULATION,
    ),
];
...
fn hashlife_diagnostic_symmetry_gate_promotion_candidate() {
    ...
    let deep_gc = pattern_by_name("gosper_glider_gun").or_invariant("required value");
